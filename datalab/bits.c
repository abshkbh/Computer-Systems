...
int divpwr2(int x, int n) {
 /*Add bias if x < 0 and perform right shift */    
    ...
    int checkmsb = msb + (~0);
    return ((checkmsb&result1)|(~checkmsb&result2)); //Different results based on sign of number  
}
/* 
...
/*Retrieve msb if 1 then return 2's complement of x otherwise return x */  
   ...
   int msb = (x>>31)&0x01;
   int resultmask = msb + (~0);                         //Generates FF's if number >=0 , otherwise 0  
...
int isPower2(int x) {
 /* All powers of 2 have only one set digit and negative numbers and zero aren't considered */
  ...
  int check1 = x&(x+(~0));           //x and x-1 will give 0 if x is a power of 2
  int check2 = !x;
...
/* Count bits in pairs of 1,2,4,8,16 by generating appropriate masks */
  ...
  int mask1 = tempmask1 | (tempmask1<<16);  //Generating 0x55555555 
  int tempmask2 = 0x33 | (0x33<<8);
  int mask2 = tempmask2| (tempmask2<<16);  //Generating 0x33333333 
  int tempmask3 = 0x0f | (0x0f<<8);
  int mask3 = tempmask3 | (tempmask3<<16);  //Generating 0x0f0f0f0f 
  int mask4 = 0xff | (0xff<<16);           //Generating 0x00ff00ff
  int mask5 = 0xff | (0xff<<8);            //Generating 0x0000ffff
  result = (x&mask1)+((x>>1)&mask1);             //Finding 1's in odd and even positions 
  result = (result&mask2)+((result>>2)&mask2);   //Finding 1's in sets of 2 
  result = (result&mask3)+((result>>4)&mask3);   //Finding 1's in sets of 4 
  result = (result&mask4)+((result>>8)&mask4);   //Finding 1's in sets of 8 
  result = (result&mask5)+((result>>16)&mask5);  //Finding 1's in sets of 16 
...
/* Retrieve msb , if 1 then mask sign bit in result and if NaN then return number as it is */
  ...
  if(exponent == 0xff && fraction!=0)   //NaN case
    result = uf;
...
/* Find exponent value by division loop , if fraction bits more than 23 round of using Bankers Algorithm */
  ...
  if(msb)                //Retrieve absolute value
    arg = -x;  
   
  quotient = arg;
  
  while(quotient>1)        //Loop to find 2^power required for number
 {   
  ...
  if(roundingbits>=0)                  //Generate bits for finding precision if fraction bits exceed 23
  {
  ...
  if(precisionbits==0x80)              //Round to even if precision bits are of the form 0b10000000
 {
  ...
  if(precisionbits>(0x80))          //Round to next highest if precision bits greater than 0b10000000
{
